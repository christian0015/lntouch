...
export default function Story() {
  ...
  const deskFillRef  = useRef(null); // vertical bar fill
  const deskPctRef   = useRef(null); // percentage text
  const deskHintRef  = useRef(null); // scroll hint overlay
  ...
  useEffect(() => {
    ...
    // ════════════════════════════════════════════════════════════════════
    // DESKTOP — lerp damping, progress bar, scroll hint
    // ════════════════════════════════════════════════════════════════════
    mm.add("(min-width: 769px)", () => {
      const BH = window.innerHeight;
      let rawProgress = 0;      // updated by ScrollTrigger
      let smoothProgress = 0;   // lerped display value
      ...
      const driveVideo = (v, lp) => {
        ...
        if (Math.abs(v.currentTime - t) > 0.035) v.currentTime = t;
      };

      // Apply all visuals from a single smooth progress value
      const applyDesktop = (p) => {
        // ── Text track
        if (trackRef.current) gsap.set(trackRef.current, { y: -p * BH * 3 });

        // ── Videos (each covers 1/3 of range)
        ...
        // ── Text highlight — smooth fade based on distance to active
        const floatActive = p * 3; // continuous 0→3
      ...
      const st = ScrollTrigger.create({
        ...
        pin: true,
        // No scrub — we handle animation manually in RAF
          ...
          <div style={{ width: "45%", height: "100vh", flexShrink: 0, display: "flex", flexDirection: "column" }}>
            {[0, 1, 2].map((i) => (
              <div key={i} style={{ flex: 1, overflow: "hidden", position: "relative", borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                ...
                <span style={{ position: "absolute", bottom: 10, left: 14, ...L }}>0{i + 1}</span>
              </div>
            ))}
          </div>
