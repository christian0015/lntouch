...
export default function Hero() {
  ...
  // ── Scroll + progress ──────────────────────────────────────────────────────
  useEffect(() => {
    let rawP = 0, smoothP = 0, currentIndex = -1, rafId = null, hintHidden = false;
    ...
    const animateText = (text) => {
      ...
      gsap.fromTo(
        ...
        { opacity: 1, y: 0, filter: "blur(0px)", stagger: 0.022, duration: 0.65, ease: "power3.out" }
      );
    };
    ...
    const applyProgress = (p) => {
      ...
      // Mobile bar + %
      if (mobFillRef.current) mobFillRef.current.style.width = `${p * 100}%`;
      if (mobPctRef.current)  mobPctRef.current.textContent  = `${pct}%`;

      // Subtitle dots
      const activeIdx = Math.min(subtitles.length - 1, Math.floor(p * subtitles.length + 0.001));
      const updateDots = (refs) => {
        refs.forEach((dot, i) => {
          if (!dot) return;
          dot.style.width      = i === activeIdx ? "16px" : "5px";
          dot.style.background = i === activeIdx ? "rgba(255,255,255,0.72)" : "rgba(255,255,255,0.2)";
        });
      };
      updateDots(deskDotsRef.current);
      updateDots(mobDotsRef.current);
    ...
    const trigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        rawP = self.progress;
        const idx = Math.min(subtitles.length - 1, Math.floor(self.progress * subtitles.length * 0.999));
        if (idx !== currentIndex) {
          currentIndex = idx;
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-screen overflow-hidden bg-black">
      ...
      >
        {/* Subtitle index dots */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "5px", marginBottom: "4px" }}>
          {subtitles.map((_, i) => (
            <div
              key={i}
              ref={(el) => { deskDotsRef.current[i] = el; }}
              style={{
                height: "1px",
                width: i === 0 ? "16px" : "5px",
                background: i === 0 ? "rgba(255,255,255,0.72)" : "rgba(255,255,255,0.2)",
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          MOBILE PROGRESS — subtitle dots + bar + % (above ticker)
      ══════════════════════════════════════════════════════════════════════ */}
      <div
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
            {subtitles.map((_, i) => (
              <div
                key={i}
                ref={(el) => { mobDotsRef.current[i] = el; }}
                style={{
                  height: "1px",
                  width: i === 0 ? "16px" : "5px",
                  background: i === 0 ? "rgba(255,255,255,0.72)" : "rgba(255,255,255,0.2)",
