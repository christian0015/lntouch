...
import { COIFFURES, EXTRAS, AVAILABILITIES, SALON, getCoiffureBySlug } from "@/lib/data";
...
const STEPS = [
  { id: 1, label: "Style",            short: "Choisissez votre style" },
  { id: 2, label: "Extras",           short: "Finitions" },
  { id: 3, label: "Créneau",          short: "Date & heure" },
  { id: 4, label: "Contact",          short: "Qui êtes-vous ?" },
...
// ─── Step Panels (NOUVEL ORDRE: Style → Extras → Créneau → Contact → Confirmation) ───

function StepStyle({ data, onChange, onNext, onSkip, preselectedStyle }: { 
  data: any; 
  ...
  const [selected, setSelected] = useState<string | null>(data.style || preselectedStyle || null);
  ...
  // Si on a un style présélectionné, on peut passer automatiquement après un court délai
  useEffect(() => {
    if (preselectedStyle && !data.style) {
      onChange({ style: preselectedStyle });
      // Option: auto-advance après 1 seconde
      const timer = setTimeout(() => {
        onNext();
      }, 800);
      return () => clearTimeout(timer);
    }
  }, [preselectedStyle, data.style, onChange, onNext]);

  return (
    <div className="step-panel">
      <StepHeader step={1} title="Quel style vous attire ?" sub="Choisissez votre coiffure — on s'occupe du reste." />
...
function StepExtras({ data, onChange, onNext, onBack }: { 
  ...
  return (
    <div className="step-panel">
      <StepHeader step={2} title="Des finitions particulières ?" sub="Optionnel — personnalisez votre expérience." />
...
function StepCreneau({ data, onChange, onNext, onBack }: { 
  ...
  return (
    <div className="step-panel">
      <StepHeader step={3} title="Choisissez votre moment." sub="Les créneaux grisés sont déjà pris — les autres vous attendent." />
...
function StepContact({ data, onChange, onNext, onBack }: { 
  ...
  return (
    <div className="step-panel">
      <StepHeader step={4} title="Entrons en contact." sub="Un email ou un WhatsApp — on vous confirme sous 12h." />
...
interface ProcessProps {
  slug?: string; // Slug optionnel pour pré-sélectionner un style
}

export default function Process({ slug }: ProcessProps = {}) {
  const sectionRef   = useRef<HTMLElement>(null);
  const leftRef      = useRef<HTMLDivElement>(null);
  const rightRef     = useRef<HTMLDivElement>(null);
  const titleLineRef = useRef<HTMLDivElement>(null);

  // Récupérer le style présélectionné si slug est fourni
  const preselectedCoiffure = slug ? getCoiffureBySlug(slug) : null;
  const preselectedStyleId = preselectedCoiffure?.slug || null;

  const [step, setStep]   = useState(1);
  const [formData, setFormData] = useState({
    email: "", phone: "",
    style: preselectedStyleId || null,
    extras: [], customNote: "",
    date: null, time: null,
  });
  ...
  // Scroll vers le haut sur mobile quand l'étape change
  useEffect(() => {
  ...
  const handleSubmit = useCallback((data: any) => {
    ...
    // TODO: Appel API pour envoyer la réservation
  }, []);

  // Rediriger à l'étape 1 si on a un slug mais que l'étape n'est pas sur Style
  useEffect(() => {
    if (preselectedStyleId && step === 1 && formData.style) {
      // L'étape Style est déjà validée, on pourrait passer automatiquement à l'étape suivante
      // Mais on laisse l'utilisateur confirmer ou modifier
    }
  }, [preselectedStyleId, step, formData.style]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* NOUVEL ORDRE: Style → Extras → Créneau → Contact → Confirmation */}
          {step === 1 && (
            <StepStyle
              data={formData}
              onChange={update}
              onNext={() => setStep(2)}
              onSkip={() => setStep(2)}
              preselectedStyle={preselectedStyleId}
            />
          )}
          {step === 2 && (
            <StepExtras
